...
export function runFullQuality(args: FullRunArgs): Promise<FullRunResult> {
  ...
  const promise = (async () => {
    try {
      ...
      if (!response.ok) {
        throw new Error(payload.error || "Could not analyze Grant profile quality.");
      }
      ...
      const message = cause instanceof Error ? cause.message : "Could not analyze Grant profile quality.";
      error = message;
      if (!args.readOnly) {
