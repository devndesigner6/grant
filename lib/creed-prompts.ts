...
export const CREED_PROMPTS = [
  {
    ...
    text: "Read my Grant profile with read_creed, then introduce me in a few tight sentences the way a sharp new collaborator would after reading my profile.",
  },
  {
    name: "tighten-my-creed",
    description:
      "Review my Grant profile and propose tightening or pruning where it has drifted.",
    text: "Read my Grant profile with read_creed, then look for anything vague, stale, duplicated, or contradictory. Propose narrowly-scoped tightening or pruning with the creed_* tools, following the contract. If nothing durable needs changing, say so and propose nothing.",
  },
] as const;
...
export function buildComposePrompt(sections: CreedSection[]): string {
  ...
  const draft = sections
    ...
    .join("\n");
  return [
    "I just finished onboarding for Grant, a personal context profile my AI assistants read so they always know me. Below is a rough starter draft built from a few onboarding questions. Using this draft plus everything you already know about me from our history, write my full Grant profile: sharp, durable, concrete, with no filler or hedging.",
    "",
    "Output ONLY the finished Grant profile as markdown inside a single fenced code block. Use exactly these headings, in this order, and do not add or remove sections:",
    "",
    headings,
    "",
    "For every section, include a short `### Graph Tags` subsection near the end. Graph Tags are only references to other sections in this Grant profile, written as hashtags for real section names from the heading list above, such as `#Goals`, `#Work`, or `#Preferences`.",
    "",
...
export function buildCompanyComposePrompt(
  ...
  const draft = sections
    ...
    .join("\n");
  return [
    `I'm setting up a shared Grant company workspace for ${name}, one context file our whole team and every AI agent we connect reads before it acts. Using this draft plus what you know about the company, write our company Grant profile: sharp, durable, concrete, with no filler or hedging.`,
    "",
    "Output ONLY the finished Grant profile as markdown inside a single fenced code block. Use exactly these headings, in this order, and do not add or remove sections:",
    "",
    headings,
    "",
    "For every section, include a short `### Graph Tags` subsection near the end. Graph Tags are only references to other sections in this company Creed, written as hashtags for real section names from the heading list above, such as `#Projects`, `#People`, or `#Agent Rules`. Use 2-4 useful related-section tags per section. Do not create hashtags for tools, apps, brands, themes, clients, projects, or labels unless they are actual section headings.",
    "",
