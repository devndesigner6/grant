...
Grant is a free personal and team context platform. A user maintains one
private Grant profile, connects agents through MCP and OAuth, and controls
which sections agents can read, propose to, or edit.

- Personal workspaces have one owner and one profile. Company workspaces are
  shared Grant profiles with owner, admin, and member roles plus per-section
  permissions and invitations.
- The editor supports Markdown import and export, rich text, proposals, direct
  edits, archive and restore, activity, versions, and Nexus graph references.
- Grant is BYOK-only for OpenRouter. Personal users and company owners save an
  encrypted OpenRouter key in Settings. Missing-key guidance is: "Add your
  OpenRouter key in Settings."
- Grant has no subscriptions, credits, billing, Stripe, blockchain, x402, or
  payment flows.
- MCP is the primary connection path at `/mcp`. It has OAuth 2.1 discovery,
  dynamic client registration, PKCE, consent, scoped bearer tokens, revocation,
  rate limits, and section-level permissions.
- The CLI source remains in `packages/creed-cli/` for folder compatibility.
  Its public package is `@devndesigner/grant-cli` and its executable is
  `grant`. It uses `GRANT_CONFIG_DIR` and only reads legacy config when Grant
  config is absent.
- GitHub supports real personal push, pull preview, and apply. Company
  workspaces can push but cannot import from GitHub yet. Keep that limitation
  explicit.
- Invitations always create a real invite record and acceptance URL. When
  Resend is absent, managers must receive a copyable URL for manual delivery.
- Required deployment values include `NEXT_PUBLIC_SITE_URL`, the Supabase URL
  and keys, `GRANT_ENCRYPTION_SECRET`, `GRANT_CSRF_SECRET`, and
  `GRANT_HEALTH_SECRET`. The health details header is
  `X-Grant-Health-Secret`.
- CSP is generated by `proxy.ts`. Production uses `GRANT_CSP_ENFORCE` unless it
  is deliberately set to `0` for controlled report-only rollout.

## Historical notes below

The remaining legacy notes have not yet been rewritten. They are historical
reference only and are not current product, billing, package, or deployment
truth. Code and the canonical Grant state above take precedence.
