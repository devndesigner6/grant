...
test("CLI mode always offers a prompt followed by the executable command", () => {
  ...
  assert.deepEqual(presentation.primary, {
    kind: "copy",
    label: "Copy prompt",
    value: "Use the Grant CLI for my personal context. Before meaningful work, run `npx @devndesigner/grant-cli --agent codex call read_creed --json`, complete browser authorization if prompted, and use the returned Grant profile to shape your response.",
  });
  assert.deepEqual(presentation.secondary, {
    kind: "copy",
    label: "Copy command",
    value: "npx @devndesigner/grant-cli --agent codex call read_creed --json",
  });
});
